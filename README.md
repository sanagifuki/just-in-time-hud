...
| 操作 | 動作 |
| --- | --- |
| Shift + Tab | メモを最前面にして入力開始。入力中は入力終了 |
| Esc | メモ入力を終了してHUD操作へ戻る |
| メモ外をクリック | メモ入力を終了してクリック先の操作を続行 |
| タイトル部ドラッグ | メモパネルを移動 |
| 右クリック | `最前面に表示`メニューを表示 |

メモ内容は起動中だけ保持され、ファイルには保存されません。
...
直近詳細、お気に入り、メモの各パネルは、右クリックメニューの `最前面に表示` で他のパネルより手前へ移動できます。検索HUDは右クリックをリスト移動に使用するため、このメニューの対象外です。

メモパネルは、一時的な作業内容を入力するための簡易メモです。`Shift + Tab`で直接フォーカスでき、再度`Shift + Tab`または`Esc`でHUD操作へ戻ります。
